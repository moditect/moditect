...
import org.moditect.dependency.CompileScopeDependencySelector;
...
public class ArtifactResolutionHelper {
    ...
    public Artifact resolveArtifact(ArtifactConfiguration artifact) throws Exception {
    ...
    public Artifact resolveArtifact(Artifact inputArtifact) throws Exception {
     ...
     */
    public List<DependencyNode> getCompilationDependencies(Artifact inputArtifact,
                                                           ArtifactResolver artifactResolver) throws Exception {
        try {
            final CollectRequest collectRequest = new CollectRequest( new Dependency( inputArtifact, "compile" ),
                    remoteRepos );
            final DefaultRepositorySystemSession sessionWithProvided = artifactResolver.newSession();
            ...
            final CollectResult collectResult = repoSystem.collectDependencies( sessionWithProvided, collectRequest );
            final PreorderNodeListGenerator nlg = new PreorderNodeListGenerator();
            collectResult.getRoot().accept( nlg );

            final List<DependencyNode> dependencies = nlg.getNodes();
            ...
            throw new Exception( "Couldn't collect dependencies of artifact " + inputArtifact, e );
        }
    }
}
